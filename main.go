...
func main() {
	fullGroupSchedule, err := schedule.GetFullGroupSchedule("ПИбд-21")
	if err != nil {
		log.Fatal(err)
	}

	weekGroupSchedule, err := schedule.ParseCurrWeekGroupSchedule(fullGroupSchedule, "ПИбд-21")
	if err != nil {
		log.Fatal(weekGroupSchedule)
	}

	groupSchedule, err := schedule.GetImgByWeekGroupSchedule(weekGroupSchedule, "ПИбд-21", 0)
	if err != nil {
		log.Fatal(err)
	}

	fmt.Print(groupSchedule)
